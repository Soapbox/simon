...
var cp = require('child_process');	// Child Process (for running shell commands). http://nodejs.org/docs/latest/api/child_process.html
var eol = require('os').EOL;		// End of Line character
var extend = require('lodash').extend; // Lodash
var glob = require('glob');

var message = require('./helpers/message');
var promise = require('./helpers/promise');
var interactivePrompt = require('./prompt');
...
Simon.prototype = {
	...
	*/
	...
	/**
		...
		@return	{Array} 1st element is command to run, 2nd is the args
	*/
	_vagrantCommand: function (command, args) {
		...
		// The run command, split on whitespace just in case
		var vagrantArgs = config.run.split(/\s+/);

		// Actual command to run
		vagrantArgs.push(
			'cd ' + config.dir + ' && ' + command + ' ' + args.join(' ')
		);

		return [config.alias,	vagrantArgs];
	},
	...
	*/
	exec: function (command, args, opts) {
		...
		local = typeof local === 'undefined' ? !!this.options.local : !!local;

		// Make sure there's a default
		args = args || [];

		// Make sure this is a proper array
		args = typeof args === 'string' ?
			args.split(/\s+/) : Array.prototype.slice.call(args);

		if (!local) {

			// Execute this command on the Vagrant VM
			command = this._vagrantCommand(command, args);
			args = command[1];
			command = command[0];
		}

		message.info('Running ' + (command + ' ' + args.join(' ')).bold);

		// Run the task
		var spawn = sync ? cp.spawnSync : cp.spawn;
		...
		if (sync) {
			// Task should be an event emitter
			var completedTask = task;
			var emitter = new events.EventEmitter();
			setImmediate(function () {
				if (completedTask.error) {
					emitter.emit('error', completedTask.status);
				}
				emitter.emit('close', completedTask.status);
			});
			task = emitter;
		} else {
	...
	*/
	execNodeFile: function (fileName, args) {
		...
		args = [fileName].concat(Array.prototype.slice.call(args));
		return this.exec('node', args, {local: true});
	},
	...
	*/
	vagrant: function () {
		var config = this.options.vagrant;
		// ALWAYS execute vagrant locally
		return this.exec(config.alias, arguments, {local: true});
	},
	...
	*/
	npm: function () {
		...
		return this.exec('npm', args, {local: true});
	},
	...
	*/
	composer: function () {
		var args = Array.prototype.slice.apply(arguments),
			command = 'composer';
		args.push('--ansi'); // colors
		if (this.options.hhvm) {
			command = 'hhvm $(which composer)';
		}
	...
	*/
	...
	/**
		Call git within the VM
		@method git
		@return {ChildProcess}
	*/
	git: function () {
		return this.exec('git', arguments, {
			local: true,
			sync: true
		}); // Always run git locally, synchronously.
	},
	...
	*/
	...
	/**
		Clear out Laravel storage folder cache
		TODO: Make this asynch and uses promises for everything
	*/
	clearCache: function () {
		var dir = process.cwd() + '/app/storage';
		var files = glob.sync('**/*', {
			cwd: dir
		});
		files.map(function (file) {
			...
			if (stats.isDirectory()) { return; } // don't delete directories
			fs.unlinkSync(file);
		});
		message.success('Application file cache cleared');

		return this.artisan('cache:clear');
	},
	...
	ssh: function () {
		...
		return this.exec(command, args, {local: false});
	},
	...
	*/
	prompt: function () {
		...
		this._prompt.trigger(); // Ask immediately
