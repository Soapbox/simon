var fs = require('fs'),				// File system
	...
	_ = require('underscore');		// Underscore.js (http://underscorejs.org)
...
Simon.prototype = {
	...
	*/
	init: function () {
		...
		process.on('exit', function () {
			simon._runningTasks.map(function (task) {
				if ('kill' in task) {
					task.kill();
				}
			});
	...
	*/
	exec: function (command, args, callback, local) {
		...
		var simon = this, runningTasks = this.runningTasks;
		...
		task.on('close', function (code) {
			...
			if (code) {
				message.warning('Task may not have completed successfully');
			} else {
				message.success('Task completed successfully');
			}
			// Remove this task from the running tasks upon completion
			simon.runningTasks = runningTasks.splice(runningTasks.indexOf(task), 1);
		});
	...
	runTasks: function (tasks, _emitter) {
		...
		if (!emitter) {
			...
			emitter.addListener('done', function () {
				message.success('All tasks completed successfully!');
	...
	*/
	help: function () {
		...
		console.log('    Note: Command-line options may not be available in interactive mode');
	},
